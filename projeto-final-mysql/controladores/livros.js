const { findByEditora } = require("../servicos/livrosMysql")
...
async function salvarLivro(req, res, db) {
    try {
        ...
        if (livro.autor && !idValido(livro.autor)) {
            res.status(422)
            return res.send({erro: "ID do autor inválido"})
        }
...
async function atualizarLivro(req, res, db) {
    try {
        ...
        if (livroAtualizado.autor && !idValido(livroAtualizado.autor)) {
            res.status(422)
            return res.send({erro: "ID do autor inválido"})
        }
...
async function getLivrosPorEditora(req, res) {
    ...
    if (!nomeValido(editora)) {
        res.status(422)
        return res.send({erro: "Editora inválida"})
    }

    const livros = await findByEditora(editora)
    if (livros && livros.length > 0) {
        res.send(livros)
        return
    }
    res.status(404)
    res.send({erro: "Livros não encontrado"})
}
